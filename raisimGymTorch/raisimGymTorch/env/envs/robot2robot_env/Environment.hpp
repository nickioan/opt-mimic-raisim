...
class ENVIRONMENT : public RaisimGymEnv {
 ...
 public:
  ...
  explicit ENVIRONMENT(const std::string& resourceDir, const Yaml::Node& cfg, bool visualizable) :
    ...
    phaseDistribution_ = std::uniform_int_distribution<int>(0,0); //max_phase_);
  ...
  void setReferenceMotionTraj() {
    Eigen::Matrix<double, 33, 1> traj_t;
    ...
    ref_foot_contact_ << traj_t.segment(29, 4);
  ...
  void computeTrackingError(const Eigen::MatrixXd actionDouble) {
    ...
    foot_reward_ = 1.0;
    for (int k = 0; k < 4; ++k) {
      if (foot_contact_state[k] != ref_foot_contact_[k]){
        foot_reward_ = 0.0;
        break;
      }
    }
 ...
 private:
  ...
  Eigen::Vector4d ref_foot_contact_;
